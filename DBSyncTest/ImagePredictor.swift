...
class ImagePredictor {
    ...
    static func createImageClassifier() -> VNCoreMLModel {
        ...
        let imageClassifierWrapper = try? DeepLabV3Int8LUT(configuration: defaultConfig)
        //try? WalletClassifier1(configuration: defaultConfig)
        //try? MobileNet(configuration: defaultConfig)
    ...
    private func visionRequestHandler(_ request: VNRequest, error: Error?) {
        ...
        predictions = observations.map { observation in
            ...
            Prediction(classification: observation.accessibilityLabel ?? "",
                       confidencePercentage: "\(observation.requestRevision)")
        }
